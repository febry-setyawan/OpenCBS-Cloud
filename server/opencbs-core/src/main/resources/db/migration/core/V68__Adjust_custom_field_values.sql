...
-- person
ALTER TABLE people_custom_fields_values ADD COLUMN IF NOT EXISTS created_by_id int, ADD COLUMN IF NOT EXISTS created_at timestamp without time zone not null default now(),
...
-- company
ALTER TABLE companies_custom_fields_values ADD COLUMN IF NOT EXISTS created_by_id int, ADD COLUMN IF NOT EXISTS created_at timestamp without time zone not null default now(),
  add column verified_by_id int,
  add column verified_at timestamp without time zone,
  add column status varchar(250),
  add column version int;
